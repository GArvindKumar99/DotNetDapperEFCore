DROP VIEW IF EXISTS [PurchaseReportDetailsView];
create view IF NOT EXISTS PurchaseReportDetailsView
AS 
...
SELECT
   ...
   IFNULL(ROUND(NETAMOUNT / 100.0, 2), 0) as NETAMOUNT,
...
WHERE
   pURCHASEORDERS.iSDELETED = 0  and isReturn=0 
GROUP BY
...
CASE
WHEN paymentMode = 'CASH' or paymentMode = 'CHEQUE' THEN paymentMode
WHEN paymentMode = 'CARD' or paymentMode = 'DIGITAL' THEN transactionType
